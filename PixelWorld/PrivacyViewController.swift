...
class PrivacyViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ServerHelper.shared.liveDataHasChanged.observeOn(MainScheduler.instance).subscribe(onNext: {[weak self] (notification) in
            ...
            guard let privacy = object as? AVObject, let isAlert = privacy.object(forKey: "isFristShow") as? Bool, let urlString = privacy.object(forKey: "privacyWebSite") as? String else { return }
    ...
    func queryData() {
        ...
        query.findObjectsInBackground {[weak self] (objs, _) in
            guard let this = self, let objc = objs?.first as? AVObject, let urlString = objc.object(forKey: "privacyWebSite") as? String else { return }
            if let url = URL(string: urlString) {
                this.privacyWeb.load(URLRequest(url: url))
            } else {
                HUD.flash(.label("请设置正确的URL地址"), delay: 2)
        ...
        tiggerCount.subscribe(onNext: {[weak self] (count) in
            if count == 3 {
                ...
                let confirmAction = UIAlertAction(title: "验证", style: .default) {[weak alertVC] (_) in
                    ...
                    if textField.text == ServerHelper.shared.adminPwd {
